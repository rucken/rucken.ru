...
export class DynamicPageComponent implements OnInit {
  ...
  constructor(
    ...
    this.page$ = this.activatedRoute.parent.params.pipe(
      map((params) => RUCKEN_SITE.getPageByName(params.pageName))
    );
    this.pageName$ = this.activatedRoute.parent.params.pipe(
      map((params) => params.pageName)
    );
  }
  ngOnInit(): void {
    const host =
      ...
      (this.request &&
        ...
        (this.request.headers.origin ||
          (this.request.headers.host
            ? (this.request.headers.host.indexOf('localhost') === 0
                ? 'http://'
                : 'https://') + this.request.headers.host
            : undefined))) ||
      RUCKEN_SITE.brand.domain;
    this.page$.pipe(untilDestroyed(this)).subscribe((page) => {
      if (page) {
        this.meta.setTitle(
          [page.description, RUCKEN_SITE.brand.title].join(' - '),
          true
        );
        ...
        if (page.type === 'modules') {
          const firstImage = page.modules
            .map((module) => module.images[0])
