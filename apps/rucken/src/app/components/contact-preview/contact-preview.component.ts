...
import { DynamicFormBuilder, DynamicFormGroup, transformValidationErrors } from 'ngx-dynamic-form-builder';
...
export class ContactPreviewComponent {
    ...
    constructor(private httpClient: HttpClient, private angulartics2: Angulartics2) {
        ...
        this.form.valueChanges.pipe(untilDestroyed(this)).subscribe(() => this.form.clearExternalErrors());
    }
    onSendMessage(form: DynamicFormGroup<ContactMessageModel>) {
        ...
        if (form.valid) {
            const sendMessage$ = this.httpClient.post(this.page.handlerUrl, classToPlain(form.object));
            sendMessage$
                .pipe(
                    catchError((error) => {
                        this.angulartics2.eventTrack.next({
                        ...
                        if (error.error && error.error.message && Array.isArray(error.error.message)) {
                            form.setExternalErrors(transformValidationErrors(error.error.message));
                        } else {
                            const message = (error && error.message) || (error && error.error && error.error.message);
                            this.alert = {
                                type: 'danger',
                                message,
                            };
                ...
                .subscribe((data: any) => {
                    ...
                    const message = (data && data.message) || 'Message sent successfully!';
                    this.alert = {
                        type: 'success',
                        message,
                    };
